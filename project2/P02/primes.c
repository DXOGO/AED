...
int main(void)
{
  ...
  int *sieve = (int *)malloc((size_t)(limit +1) * sizeof(int));  // antes n tinhamos '*sizesof(int)' e dava crash pq pediamos
  							   // ao programa 1000000 de bytes em vez de ints(cada int tem 4 bytes)
  if(sieve == NULL)
    return 1;
